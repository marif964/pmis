...
<div class="container-fluid">
    <form action="" id="project-form">
        ...
        <div class="form-group">
            <label for="name" class="control-label">Project Start Date</label>
        ...
        <div class="form-group">
            <label for="name" class="control-label">Project End Date</label>
            <input type="date" name="name" id="name" class="form-control form-control-border" placeholder="Enter Project Name" value ="<?php echo isset($name) ? $name : '' ?>" required>
        </div>
        <div class="form-group">
								<label for="" class="control-label">Project Sector</label>
             ...
             <div class="form-group">
								<label for="" class="control-label">Project Donor</label>
                                <select name="did" id="did" class="form-control form-control-sm select2">
                                    <?php 
                                    $type = $conn->query("SELECT id,name FROM `project_donors` WHERE 1");
                                    while($row= $type->fetch_assoc()):
                                        if(!isset($did)){
                                            $did = $row['id'];
                                        }
                                        if($did == $row['id'])
                                            $emp = $row['name'];
                                    ?>
                                    <option value="<?= $row['id'] ?>" <?= isset($did) && $did == $row['id'] ? "selected" : "" ?>><?= $row['name'] ?></option>
                                    <?php endwhile; ?>
                                </select>
			</div>
